...
Customizations are deliberately kept out of component overrides wherever possible: CSS applied to Starlight's default output, or route data adjusted through Starlight's route middleware, survives an upgrade, while an overridden component has to be re-checked against upstream every time. Only `Sidebar` is a wholesale replacement of a default and `SiteTitle` is a small one; the right sidebar is removed by taking the TOC off the route data (`src/routeData.ts`) rather than by replacing the layout component, and the TOC (`PageToc`) is rendered by components of our own rather than by overriding Starlight's, so upstream changes cannot silently break it. The few places that do depend on Starlight internals are listed in `tests/unit/upstream-assumptions.test.ts`, which fails when an upgrade invalidates one of them.

### Structure

- `src/pages/ja/index.astro`, `src/pages/en/index.astro` — top page per locale, a generated list of the titles of every document in that locale (add one file per locale); both render `src/components/DocumentIndex.astro`
...
- `src/i18n.ts` — single source of truth for the supported locales (`ja` default / `en`) and `generateDocsId`/`localeOfDocsId`, which remap a content path like `a/en/index.md` to the Starlight slug `en/a` so the doc-then-locale directory layout above still produces Starlight's expected locale-prefixed URLs (`/ja/a/`, `/en/a/`). Every locale carries its prefix, the default one included, so no page sits at the site root
- `src/content.config.ts` — wires `generateDocsId` into the loader below
- `src/docsLoaderWithTitleFromHeading.ts` — wraps Starlight's `docsLoader` so a page title comes from the leading H1 of the body instead of a `title` frontmatter field. The H1 lookup itself lives in `src/titleFromHeading.ts`, which parses the body as Markdown (rather than searching it with a regular expression) so a `#` line that is not a heading — a comment in a code block or in the frontmatter, a shebang — is not mistaken for one. `src/remarkStripLeadingHeading.ts` then removes that H1 from the rendered body so it isn't shown twice; it locates the heading with the same `leadingHeadingIndex` the title was taken from, so the title and the removed heading can never disagree
- `remark-breaks` (a dependency, wired up in `astro.config.mjs`) — renders a plain line break inside a paragraph as `<br>` instead of collapsing it into a space, so a document reads the way it was written (only text is rewritten, so code blocks, inline code, and math are left alone; `tests/e2e/build.test.ts` checks both)
- `astro.config.mjs` — Starlight/Pagefind/KaTeX/`base`/i18n (`locales: starlightLocales` and `defaultLocale` from `src/i18n.ts`) configuration, plus the `redirects` entry that sends the site root (`base` itself, which no locale occupies) to the top page of the default locale, `/ja/`. A static build writes an HTML page with a `<meta http-equiv="refresh">` there and an adapter that supports redirects can turn the same entry into a real 3xx; the destination is written against the base the site is built under (see `src/portableBase.ts`), so it carries the deployment path like every other URL
...
- `src/portableBase.ts` — integration that makes the built site placeable in any directory of the web server, including one whose name contains non-ASCII characters. Astro compares `base` as it is written in the config against paths that are already percent-encoded (a request path, the URL Vite emitted for an asset, the path of the page being rendered), so a `base` such as `/日本語ディレクトリ/` never matches: the build fails, the dev server answers 404, and Starlight generates broken links. Nothing has to know the deployment path while the site is being built, so the build runs under a plain ASCII placeholder base (`/__base__/`) and the last step of the build replaces that placeholder with `base` in the output — page links, asset URLs, CSS `url()`s, `import.meta.env.BASE_URL` inlined into scripts, canonical URLs and the sitemap, all in one place. It is the last entry in `integrations` for that reason: Starlight inserts its own (Pagefind, sitemap, …) right after itself, and the rewrite has to see what they wrote. A file that is not text yet carries the placeholder fails the build rather than being rewritten
- `src/components/Sidebar.astro` — override that shows a table of contents (TOC) in the left sidebar instead of a page list
- `src/routeData.ts` — Starlight route middleware that takes the TOC off the route data and hands it to the left sidebar through `Astro.locals.pageToc`. Starlight renders the right sidebar (the TOC column) only when `starlightRoute.toc` is set, so clearing it produces the two-pane layout (left TOC + body) without overriding the layout component, and leaves the widths and the mobile TOC handling to Starlight and the theme
...
- `src/components/PageToc.astro`, `src/components/PageTocList.astro` — the TOC itself, rendered from `Astro.locals.pageToc` (see `src/routeData.ts`) instead of using Starlight's `<TableOfContents />`. Starlight's component ships its highlighting script with its markup and the two cannot be separated, so the markup is kept here as well (it mirrors the upstream one)
...
| Path | What it covers |
| --- | --- |
| `tests/unit/` | The pure logic this project adds on top of Starlight: `generateDocsId` / `localeOfDocsId` slug mapping and locale-config consistency (`i18n.test.ts`), the rewrite that writes the deployment path into the built output (`portableBase.test.ts`), H1 title extraction, including the `#` lines that are not headings and its agreement with the H1 removal (`titleFromHeading.test.ts`), the remark plugins (`remarkStripLeadingHeading.test.ts`, `remarkHtmlImage.test.ts`), and the TOC highlighting rule at its boundary values (`activeTocIndex.test.ts`). `upstream-assumptions.test.ts` is different in kind: it reads the installed Starlight / theme sources and asserts that the internals the customizations lean on are still there. A failure there means an upgrade needs following up on, not that something is broken. |
...
| `tests/e2e/` | A real `astro build`. `build.test.ts` builds a temporary project whose `src/content/docs/` is replaced by `tests/fixtures/docs/` and asserts on the generated HTML — URLs (every page under its locale prefix, the site root a redirect to the default locale), titles, no duplicated `<h1>`, line breaks kept as `<br>` (but not inside code blocks), hashed image assets from both `![]()` and raw `<img>`, KaTeX output, the per-locale document list, `lang` attributes, the TOC sidebar, the Pagefind index, and the deployment path written into the output. `build-errors.test.ts` checks that misplaced documents fail the build with a message that names the offending file. `build-nonascii-base.test.ts` builds with a Japanese `base` and checks every place percent-encoding it would break: image URLs, the language switcher, the locale of a page rendered by `<StarlightPage>`, the redirect at the site root, and the links of the document list. Both builds also assert that no file is left carrying the placeholder base. |
...
`base` itself holds no page: it is an HTML page that redirects to the top page of the default locale (`/starlight-demo/ja/`, see the `redirects` entry in `astro.config.mjs`), which every static server serves as the directory index. A host that can redirect on its own can be pointed at that URL directly instead.

A `base` containing non-ASCII characters, such as `/日本語ディレクトリ/`, builds and deploys as well; URLs then carry it percent-encoded. Astro itself never sees such a path: the site is built under an ASCII placeholder base and `base` is written into the output at the end of the build (see `src/portableBase.ts`). The one thing that changes is `npm run dev`, which serves the site under that placeholder (`http://localhost:4321/__base__/`, printed at startup) rather than under `base` — it serves pages, not the built output, so there is nothing to rewrite. `npm run preview` serves the built output and therefore uses `base` as configured, which means it cannot serve a `base` that URLs encode (Astro's own servers match the request path against the raw string); check that one with the static server above, symlinking `dist` under the directory name of the base.
