...
- `src/localeRootRedirect.ts` — integration that redirects the site root (`base` itself, which no locale occupies) to the top page of the default locale, `/ja/`. It registers an Astro `redirects` entry, so a static build writes an HTML page with a `<meta http-equiv="refresh">` there and an adapter that supports redirects can turn it into a real 3xx. A page under `src/pages/` would be the obvious alternative, but Astro's i18n router answers 404 for a path with no locale unless it recognises it as the site root — which it does by comparing the path against the raw `base`, so a `base` that URLs percent-encode would drop that page from the build
...
| Path | What it covers |
| --- | --- |
| `tests/unit/` | The pure logic this project adds on top of Starlight: `generateDocsId` / `localeOfDocsId` slug mapping and locale-config consistency (`i18n.test.ts`), the destination of the site root redirect (`localeRootRedirect.test.ts`), H1 title extraction including the code-fence exclusion (`titleFromHeading.test.ts`), the remark plugins (`remarkStripLeadingHeading.test.ts`, `remarkHtmlImage.test.ts`, `remarkBreaks.test.ts`), and the TOC highlighting rule at its boundary values (`activeTocIndex.test.ts`). `upstream-assumptions.test.ts` is different in kind: it reads the installed Starlight / theme sources and asserts that the internals the customizations lean on are still there. A failure there means an upgrade needs following up on, not that something is broken. |
